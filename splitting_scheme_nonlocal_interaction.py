...
plt.subplot(222)
plt.plot(x, np.abs(u[0,1:nx+1])**2, label="Condition initiale", linewidth=1.5)
plt.plot(x, np.abs(u[-1, 1:nx+1])**2, "r", label="Solution splitting", linewidth=1.5)
plt.plot(x, np.abs(u_det[-1, 1:nx+1])**2, "g", label="Solution déterministe", linewidth=1.5)
plt.plot(x, np.abs(u_duhamel[-1, 1:nx+1]), "--r", label="Solution duhamel", linewidth=1.5)
plt.plot(x, np.abs(u_sEXP[-1, 1:nx+1])**2, ":r", label="Solution sEXP", linewidth=1.5)
plt.legend(fontsize="small")
plt.title("Graphique de |u|**2")
plt.xlabel("x")
plt.ylabel("u")
plt.grid(True, alpha=0.3)

plt.subplot(223)
plt.plot(x, np.abs(u[0, 1:nx+1])**2, label="Temps 0", linewidth=1.5)
plt.plot(x, np.abs(u[int(nt/2), 1:nx+1])**2, label="Temps "+str(int(nt/2)*tau), linewidth=1.5)
plt.plot(x, np.abs(u[-1, 1:nx+1])**2, label="Temps "+str(T), linewidth=1.5)
plt.legend(fontsize="small")
plt.title("Graphique de |u|**2 splitting")
plt.xlabel("x")
plt.ylabel("u")
plt.grid(True, alpha=0.3)
