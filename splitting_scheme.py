...
Vx = np.zeros(nx+2, dtype=complex)
Vx[1:nx+1] = V(x)
Vx[0]=Vx[nx]
Vx[nx+1]=Vx[1]

S = np.zeros(nx+2, dtype=complex)
S[1:nx+1] = noyau_chaleur(tau, nx)
S[0]=S[nx]
S[nx+1]=S[1]
...
for i in range(nc): 
    ...
    W = np.zeros((nt,nx+2), dtype=complex)
    W[:,1:nx+1] = Wq(nt, tau, nx, x) # shape(nt, nx)
    W[:,0] = W[:,nx]
    W[:,nx+1] = W[:,1]
    ...
    for n in range(1, nt):
        ...
        #     s = tau*(l-1)
        #     integrale1 += np.fft.ifft(noyau_chaleur(temps-s, nx)*np.fft.fft(Vx*u_duhamel[l-1,1:nx+1])) * tau
...
for m in range(1, nt):
    ...
    u_det[m, 1:nx+1] = u_lin_det[1:nx+1]
...
plt.legend(fontsize="small")
plt.title("Graphique de Real(u) au fil de t")
plt.xlabel("x")
plt.ylabel("u")
plt.grid(True, alpha=0.3)
